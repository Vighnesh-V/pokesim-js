...
socketServer.on('connection', function (socket) {
  ...
  socket.on('trade', function (data) {
      pDb.getPlayer (data.me, function (err, player) {
        pDb.getPlayer (data.them, function (err, enemy) {
          var player1prim = player[0].prim;
          var player2prim = enemy[0].prim;

          var player1seco = player[0].seco;
          var player2seco = enemy[0].seco;
          ...
          pDb.updatePlayer(playerData, function (err) {
            pDb.updatePlayer(enemyData, function (err) {
            });
          });
  ...
  socket.on('get_enemy_secondary', function (data) {
    if (data.e > 1) {
      pDb.getPlayer(data.me, function (err, result) {
          ...
          socket.emit('game_over_winner', {me: data.me, them: data.name});
      });
    } else {
      pDb.getPlayer(data.name, function (err, result){
          var id = result[0].seco.id;
